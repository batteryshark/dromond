...
PROFILES = {
    "cheap": {"backend": "opencode", "tier": 1},
    "big": {"backend": "opencode", "tier": 3, "priority": 10},
    "bigger": {"backend": "opencode", "tier": 3, "priority": 5},
}
...
class DispatchTestCase(ResolverFixture):
    ...
    def test_without_one_the_top_tier_3_profile_is_staffed(self) -> None:
        self.run_branch({"app.py": "print('branch')\n"})
        self.add_run()
        new_id, _ = self.dispatch({"profiles": PROFILES})
        self.assertEqual("bigger", self.db_run(new_id)["profile"])  # priority 5 < 10

    def test_no_profile_at_all_refuses_on_stderr(self) -> None:
        self.run_branch({"app.py": "print('branch')\n"})
        self.add_run()
        new_id, err = self.dispatch({"profiles": {"cheap": PROFILES["cheap"]}})
        self.assertIsNone(new_id)
        self.assertIn("tier = 3", err)
