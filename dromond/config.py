...
Merge order: built-in defaults <- ~/.config/dromond/config.toml <-
that file's [project."<projectId>"] table (shallow, per table). Per-project
settings key on Work's immutable projectId and live in Dromond's own config
...
# The names Dromond accepted before the numbers, plus Orchestra's own. Read
...
DEFAULT_CONFIG = f"""\
# Dromond profiles + settings. The only file: ~/.config/dromond/config.toml
#
...
# loses expand-in-place detail. Run it with `dromond traces prune`.
...
# on its own — there is no concurrency cap anywhere in Dromond (§4).
...
# Off unless enabled. agent_identity is the X-Work-Agent name Dromond posts
...
# items. poll_interval is the fallback heartbeat for `dromond sweep
# --watch`, in seconds.
[work]
enabled = false
...
# A Nod issuer token is scoped to exactly ONE channel, so Dromond holds two
...
# Any of those keys can be overridden per-run by DROMOND_NOD_<KEY>, e.g.
# DROMOND_NOD_ALERTS_TOKEN. Configure one channel and not the other and only
# that other channel reports as unconfigured.
[nod]
enabled = false
secrets_file = "~/.config/dromond/nod-secrets.env"
timeout = 15
...
#                 `dromond profiles note NAME "..."` sets both

"""

LEGACY_AGENTS_ERROR = """\
dromond: {path} still uses the legacy [agents.NAME] config table.
...
dromond: {path} still carries per-project profile overrides:
{tables}
...
ENABLED_TYPE_ERROR = """\
dromond: enabled_profiles in [project."{project_id}"] ({path}) must be a list
...
def load(project_id: str | None = None) -> dict:
    ...
    for overlay in (top, per_project):
        ...
        # supervisor OR for `dromond merge`.
...
def not_enabled_error(cfg: dict, name: str) -> str:
    ...
    return (
        f"dromond: project {project_id} has not enabled profile '{name}'.\n"
...
def staff_profile(cfg: dict, name: str) -> dict:
    ...
    Call this where a run is being STAFFED — the sweeper, ``dromond
...
def profile_cfg(cfg: dict, name: str) -> dict:
    ...
    if name not in profiles:
        if not profiles:
            raise SystemExit(
                "dromond: no profiles configured. A profile is backend + model +\n"
                "effort, picked from what the harnesses report — run\n"
                "`dromond profiles discover` to see the real lists, then add a\n"
                f"[profiles.NAME] table to {paths.global_config_path()}"
            )
        raise SystemExit(
            f"dromond: unknown profile '{name}'. Configured: {', '.join(sorted(profiles))}\n"
    ...
    if not isinstance(raw, list) or not all(isinstance(d, str) and d for d in raw):
        raise SystemExit(
            f"dromond: add_dirs must be a list of directory paths "
...
def apply_worker_env(cfg: dict, env: dict[str, str], root: Path) -> dict[str, str]:
    ...
    if not isinstance(values, dict):
        raise SystemExit("dromond: [worker_env] must be a TOML table")
    updated = dict(env)
    for name, value in values.items():
        if not isinstance(name, str) or not name or "=" in name or "\0" in name:
            raise SystemExit(f"dromond: invalid worker environment name {name!r}")
        if not isinstance(value, str) or "\0" in value:
            raise SystemExit(f"dromond: worker environment value for {name} must be a string")
